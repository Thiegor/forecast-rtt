...
export default function App() {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        const { data } = await supabase.from('usuarios').select('*').eq('email', session.user.email.toLowerCase()).maybeSingle()
        setPerfil(data)
  ...
  // Login
  if (estado === 'login' || estado === 'sem-perfil') {
    return (
        ...
        <div style={{width:'100%',maxWidth:380}}>
          ...
          <div style={{background:'#161616',border:'1px solid #272727',borderRadius:10,padding:'28px 24px'}}>
            <form onSubmit={handleLogin} style={{display:'flex',flexDirection:'column',gap:16}}>
              ...
              {estado === 'sem-perfil' && <div style={{background:'rgba(227,30,36,0.1)',border:'1px solid rgba(227,30,36,0.3)',color:'#fca5a5',padding:'9px 12px',borderRadius:6,fontSize:12}}>Usuário não encontrado na base. Contate o Planejamento e Controle.</div>}
  ...
  // App principal — importado dinamicamente para evitar erro #284
  const ForecastApp = require('./pages/Forecast').default
  return <ForecastApp perfil={perfil} onLogout={handleLogout} />
}
